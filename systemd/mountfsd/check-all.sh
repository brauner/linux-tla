...
for cfg in "${cfgs[@]}"; do
    ...
    log=logs/$cfg.log
    if grep -q 'TLC threw an unexpected exception\|Parsing or semantic analysis failed\|Assertion\|OutOfMemory' "$log"; then
        result=ERROR
    ...
    printf '%-28s %-10s %-12s %-10s %-12s %s\n' "$cfg" "$expect" "$result" "$states" "${took:--}" "$mark" | tee -a logs/summary.txt
done
