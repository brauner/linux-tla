...
for cfg in "${cfgs[@]}"; do
    ...
    if [ ! -f "$log" ]; then state=never
    elif grep -q 'TLC threw an unexpected exception\|Parsing or semantic analysis failed\|Assertion\|OutOfMemory\|when writing the disk' "$log"; then
        ...
        state=violation; detail="$(sed -n 's/^\([0-9]*\) states generated, \([0-9]*\) distinct.*/\2 states/p' "$log" | tail -1), $(sed -n 's/^Finished in \(.*\) at .*/\1/p' "$log" | tail -1)"
