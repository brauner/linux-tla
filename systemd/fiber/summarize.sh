...
for log in logs/*.log; do
    ...
    if grep -q 'TLC threw an unexpected exception\|Parsing or semantic analysis failed\|Assertion\|OutOfMemory\|when writing the disk\|Semantic errors' "$log"; then result=ERROR
    elif grep -q 'Invariant .* is violated\|Temporal properties were violated\|is not a valid\|Deadlock reached' "$log"; then result=violation
